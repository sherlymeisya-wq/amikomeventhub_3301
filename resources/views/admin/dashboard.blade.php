...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
  ...
  <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
    ...
    <h3 class="text-2xl font-black">{{ $totalTransactions }}</h3>
  </div>
  ...
  <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
    ...
    <h3 class="text-2xl font-black">{{ $pendingTransactionsCount ?? 0 }} Pesanan</h3>
  </div>
</div>
    ...
    <table class="w-full text-left border-collapse">
      ...
      <tbody class="divide-y border-t">
        ...
        <tr class="hover:bg-slate-50 transition">
          <td class="px-8 py-6">
            <p class="font-bold uppercase tracking-wide text-sm">{{ $tx->customer_name }}</p>
            <p class="text-xs text-slate-400">{{ $tx->customer_email }}</p>
          </td>
          <td class="px-8 py-6 font-medium text-slate-600">{{ $tx->event->title ?? 'N/A' }}</td>
          <td class="px-8 py-6">
            ...
            @else
              <span class="px-3 py-1 bg-red-100 text-red-700 rounded-lg text-xs font-bold uppercase">{{ $tx->status }}</span>
            @endif
          </td>
        ...
        @empty
        <tr>
          <td colspan="4" class="px-8 py-6 text-center text-slate-400">Belum ada data transaksi masuk.</td>
        </tr>
        @endforelse
      </tbody>
    </table>
