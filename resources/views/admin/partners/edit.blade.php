...
<header class="flex justify-between items-center mb-10">
    <div>
        ...
        <p class="text-slate-500 font-medium">Perbarui data partner yang tampil di homepage.</p>
    </div>
...
<div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 max-w-3xl">
    <form action="{{ route('admin.partners.update', $partner->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $partner->name) }}" placeholder="Contoh: Amikom Digital"
        ...
        <div>
            ...
            <input type="url" name="logo_url" value="{{ old('logo_url', $partner->logo_url) }}" placeholder="https://example.com/logo.png"
                class="w-full px-5 py-4 rounded-3xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition">
            @error('logo_url')
                <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>
            @enderror
        ...
        <div class="flex justify-end gap-4 pt-4">
            ...
            <button type="submit" class="px-8 py-3 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 transition">Perbarui Partner</button>
        </div>
    </form>
</div>
@endsection
